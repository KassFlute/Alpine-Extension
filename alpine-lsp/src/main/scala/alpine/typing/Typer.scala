...
final class Typer(
  ...
  def visitFunction(d: ast.Function)(using context: Typer.Context): Type =
    ...
    val t: Type = context.inScope(d, { (inner) =>
      ...
      context.obligations.add(Constraint.Subtype(body, arrow.output, Constraint.Origin(d.body.site)))
  ...
  def visitApplication(e: ast.Application)(using context: Typer.Context): Type =
    ...
    val args = e.arguments.map((a) => Type.Labeled(a.label, a.value.visit(this)))
    f match
      case Type.Arrow(inputs, output) =>
        context.obligations.add(Constraint.Apply(f, args, output, Constraint.Origin(e.site)))
  ...
  def visitPrefixApplication(e: ast.PrefixApplication)(using context: Typer.Context): Type =
    val f = e.function.visit(this)
    val arg = e.argument.visit(this)
    f match
      case Type.Arrow(inputs, output) if inputs.length == 1 =>
        context.obligations.add(Constraint.Apply(f, List(Type.Labeled(None, arg)), output, Constraint.Origin(e.site)))
        context.obligations.constrain(e, output)
      case _ =>
        val fresh = freshTypeVariable()
  ...
  def visitInfixApplication(e: ast.InfixApplication)(using context: Typer.Context): Type =
    val f = e.function.visit(this)
    val left = e.lhs.visit(this)
    val right = e.rhs.visit(this)
    f match
      case Type.Arrow(inputs, output) if inputs.length == 2 =>
        context.obligations.add(Constraint.Apply(f, List(Type.Labeled(None, left), Type.Labeled(None, right)), output, Constraint.Origin(e.site)))
        context.obligations.constrain(e, output)
      case _ =>
        val fresh = freshTypeVariable()
  ...
  def visitConditional(e: ast.Conditional)(using context: Typer.Context): Type =
    ...
    val thenBranch = checkedType(e.successCase)
    val elseBranch = checkedType(e.failureCase)
    if thenBranch == elseBranch then 
      context.obligations.constrain(e, thenBranch) 
    else
      val tau = freshTypeVariable()
  ...
  def visitLambda(e: ast.Lambda)(using context: Typer.Context): Type =
    ...
    context.inScope(e, { (inner) =>
      ...
      e.output match
        case Some(t) =>
          val arrow = Type.Arrow(inputs, evaluateTypeTree(t))
        ...
        case _ =>
          val arrow = Type.Arrow(inputs, freshTypeVariable())
          context.obligations.add(Constraint.Subtype(e.body.visit(this), arrow.output, Constraint.Origin(e.site)))
          context.obligations.constrain(e, arrow)
    })
  ...
  def visitTypeIdentifier(e: ast.TypeIdentifier)(using context: Typer.Context): Type =
    resolveUnqualifiedTermIdentifier(e.value, e.site) match
      case Nil =>
        ...
        Type.Error
      case List(symbols.EntityReference(_, Type.Meta(t))) =>
        t
      case _ =>
